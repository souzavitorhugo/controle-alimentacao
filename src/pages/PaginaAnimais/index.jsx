...
import {
    listarRacas, listarEspecies,
    cadastrarAnimais, listarAnimais,
    cadastrarEspecies, cadastrarRacas,
    recuperarAnimalPorId, editarAnimais
} from '../../actions/controllers/animaisController';
...
export default function PaginaAnimais(props) {
    ...
    const handleChangeEspecies = (event) => {
        atualizaHookRacas(event.target.value);
    }

    const atualizaHookRacas = async (idEspecieAnimal) => {
        try {
            const data = await listarRacas();
            if(!!data) {
                let listaRetornoAPI = data;
                let listaFiltrada = listaRetornoAPI.filter(item => item.tipoAnimal.id == idEspecieAnimal)
                setDisabled(false);
                setListaRacas(listaFiltrada);
            }
        } catch(err){
            throw window.alert("Erro na consulta ao banco das raças, por favor tente novamente");
        }
    }
                        ...
                        <div className="container-form">
                            ...
                            <form onSubmit={formikAnimal.handleSubmit}>
                                ...
                                <div className="form-container-inputs">
                                    ...
                                    <Input
                                        id="animal"
                                        options={listaEspecies?.map(tipoAnimal => ({ value: tipoAnimal.id, label: tipoAnimal.tipo }))}
